...
function public_anketa(){
	...
	$q01 = myquery("select id from anketa where  ak_god=$ag and aktivna = 1");
	...
	if ($_POST['akcija'] == "finish" && check_csrf_token()) {
		...
		$result204=myquery ("SELECT count(*) FROM pitanje WHERE anketa_id =$id_ankete and tip_id =2");
		...
		for ($i=0; $i<$broj_esej_pitanja ; $i++){
			...
			$q590 = myquery("insert into odgovor_text set rezultat_id=$id_rezultata, pitanje_id=$id_pitanja[$i], response='$komentar[$i]'");
