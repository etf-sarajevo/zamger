	...
	foreach ($studiji as $studij => $ime) {
		for ($ponovac=0; $ponovac<=1; $ponovac++) {
			for ($nacin=1; $nacin<=4; $nacin++) {
				...
				$q40 = myquery("select count(*) from student_studij where akademska_godina=$ak_god and semestar=$semestar and studij=$studij and ponovac=$ponovac and nacin_studiranja=$nacin");
				$x = mysql_result($q40,0,0);
