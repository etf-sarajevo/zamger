...
if ($_POST['novagrupa']) {
	mysql_query("insert into grupe set naziv='".$_POST['imegrupe']."'");
	$message = "<span class=\"msg\">Nova grupa <b>".$_POST['imegrupe']."</b> je dodana.</span>";
}

if ($_POST['novistudent']) {
	mysql_query("insert into studenti set ime='".$_POST['ime']."', prezime='".$_POST['prezime']."', grupa=".$_POST['grupa'].", email='".$_POST['email']."', brindexa='".$_POST['brindexa']."'") or die(mysql_error());
	$message = "<span class=\"msg\">Student <b>".$_POST['ime']." ".$_POST['prezime']."</b> je dodan.</span>";
}

if ($_POST['izmstudent']) {
	mysql_query("update studenti set ime='".$_POST['ime']."', prezime='".$_POST['prezime']."', grupa=".$_POST['grupa'].", email='".$_POST['email']."', brindexa='".$_POST['brindexa']."' where id=".$_POST['idid']) or die(mysql_error());
...
while ($jedanred = mysql_fetch_row($result)) {
	...
	<?
	$result2 = mysql_query("select id,ime,prezime from studenti where grupa=$grupa_id order by prezime,ime");
...
if ($_GET['id'] != 0) {
	$result = mysql_query("select grupa, ime, prezime, email, brindexa from studenti where id=".$_GET['id']);
