...
if (mysql_result($q10,0,0) == 0) {
    zamgerlog("nijedna zadaæa nije aktivna, predmet pp$predmet", 3);
	niceerror("Nijedna zadaæa nije aktivna");
	return;
}



//  ODREÐIVANJE ID ZADAÆE

// Da li neko pokušava da spoofa zadaæu?
if ($zadaca!=0) {
	...
	if (mysql_result($q20,0,0)==0) {
		...
		biguglyerror("Ova zadaæa nije iz vašeg predmeta");
		return;
	}
}
...
if ($zadaca==0) {
	...
	if (mysql_num_rows($q30)>0)
		...
		if (mysql_num_rows($q40)>0)
			...
			// Ako ni ovdje nema rezultata, znaèi da je svim 
			// zadaæama istekao rok. Daæemo zadnju zadaæu.
...
// Ove vrijednosti æe nam trebati kasnije
...
if ($zadatak==0) { 
	// Prvi neuraðeni zadatak u datoj zadaæi
	// NOTE: subquery
...
$stat_tekst = array("Bug u programu", "Pregled u toku", "Zadaæa prepisana", "Bug u programu", "Pregled u toku", "Zadaæa OK");
...
while ($r21 = mysql_fetch_row($q21)) {
	...
	for ($m_zadatak=1;$m_zadatak<=$broj_zadataka;$m_zadatak++) {
		// Ako tekuæa zadaæa nema toliko zadataka, ispisujemo blank polje
...
if ($rok <= time()) {
	print "<p><b>Vrijeme za slanje ove zadaæe je isteklo.</b></p>";
	// Ovo je onemoguæavalo copy&paste u Firefoxu :(
	//$readonly = "DISABLED";
} else {
	$readonly = "";
}
...
//		biguglyeerror("Ova zadaæa nije iz vašeg predmeta");
		// student_zadaca() ce ispisati grešku
		return;
	}
	...
	// Provjera roka
	if ($rok <= time()) {
		niceerror("Vrijeme za slanje zadaæe je isteklo!");
		zamgerlog("isteklo vrijeme za slanje zadaæe z$zadaca",3); // nivo 3 - greska
		return; 
	}
	...
	// Vrsta zadaæe: textarea ili attachment
	if ($attach == 0) {
		// Odreðivanje ekstenzije iz jezika
		...
		// Temp fajl radi odreðivanja diff-a 
		$diffing=0;
		...
		if (strlen($program)<=10) {
			...
			niceerror("Greška pri slanju zadaæe. Kontaktirajte tutora.");
		}
		...
		if ($program && file_exists($program) && in_array($file_ext,$db_doz_eks)&& $provjera==0) {
			// Neæemo pokušavati praviti diff
		...
		else if($provjera==1)//ako nije selktovana niti jedna ekstenzija, u tom slucaju omogucujemo da moze poslati fajl sa bilo kojom ekstenzijom
		{
			...
			niceerror("Greška pri slanju zadaæe. Provjerite da li je ekstenzija datoteke dozvoljena. <br />Vaša ima $file_ext, a dozvoljene su:
