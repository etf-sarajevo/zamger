...
function includes_profile($title, $person, $isAdmin) {
	...
	includes_profile_add_place($drzava, $opcina);
		...
		<div class="col-md-12 text-left border rounded-3">
			<div class="container-fluid bg-white m-0 p-3 ">
				...
				<form class="p-0" action="" method="post" id="update-profile" autocomplete="off">
					...
					<?= Form::hidden('isAdmin', $isAdmin, ['class' => 'form-control', 'id' => 'isAdmin']) ?>
					...
					<div class="row">
						...
						<div class="col-md-6">
							<div class="form-group">
								...
								<?= Form::text('studentIdNr', $person['studentIdNr'] ?? '', ['class' => 'form-control form-control-sm', 'id' => 'studentIdNr', 'aria-describedby' => 'studentIdNrHelp', $isAdmin ? '' : 'readonly']) ?>
								<small id="studentIdNrHelp" class="form-text text-muted"> <? if (!$isAdmin) print "Ovaj podatak može uređivati samo administrator i/ili studentska služba" ?> </small>
							</div>
						</div>
					</div>
					<div class="row">
