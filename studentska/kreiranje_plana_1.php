...
<? 
   ...
   ?> 
    <label style="float: left;width: 120px;font-weight: bold;">Godina 
važenja:</label> 
     <select name="godina_vazenja" class="formElement" 
id="godina_vazenja" style="width: 250px;margin-bottom: 5px;"> 
      <?  
       $cnt = 0; 
       $akademske = fetch_ag(); 
      ...
      ?>   
        <option value="<?= $akg['id']?>"><?= 
nl2br($cnt . '.' . $akg['naziv'])?></option> 
      <? }  
      ?> 
     </select> 
     <br /> 
    <label style="float: left;width: 120px;font-weight: 
bold;">Studij:</label> 
     <select name="studij" class="formElement" id="studij" 
style="width: 250px;margin-bottom: 5px;"> 
      ...
      ?>   
        <option value="<?= $std['id']?>"><?= 
nl2br($cnt . '.' . $std['naziv'])?></option> 
      <? }  
      ?> 
     </select> 
     <br /> 
    <label style="float: left;width: 120px;font-weight: bold;">Semestar: 
</label> 
     <input type="text" name="semestar" value="" style="width: 
30px;"> 
     <br /> 
    <label style="float: left;width: 120px;font-weight: 
bold;">Predmet:</label> 
     <select name="predmet" id="predmet" style="width: 250px;margin-
bottom: 5px;"> 
      <?  
       $cnt = 0; 
...
nl2br($cnt . '.' . $prm['naziv'])?></option> 
    ...
    <br /> 
    <input type="submit" name="create" id="submitbutton" value="Potvrdi" 
...
?>   
    <a href="?sta=studentska/plan">PRIKAŽI PLAN</a> 
        <br/><br/> 
    <a href="?sta=studentska/kreiranje_plana">Nazad na kreiranje nove 
stavke plana</a> 
    </div> 
    </div> 
<? 
...
function processInsertPlanForm(){ 
 ...
 { 
  ...
  $errorText = createPlan($godina_id, $studij_id, $semestar, $predmet_id, 
$obavezan); 
...
function createPlan ($godina_id, $studij_id, $semestar, $predmet_id, $obavezan){ 
  ...
  if($row['zavrsni_semestar'] < $semestar ){ 
   $errorText = "Uneseni semestar je veci od ukupnog broja semestara izabranog 
studija!"; 
   return $errorText; 
 } 
  
  $query = sprintf("INSERT INTO plan_studija 
(godina_vazenja,studij,semestar,predmet,obavezan) VALUES ('%d', '%d', '%d', '%d', 
'%d')",  
      $godina_id, $studij_id, $semestar, $predmet_id, 
$obavezan); 
  ...
  if ($result == false) 
 { 
   $errorText = 'Došlo je do greške prilikom spašavanja podataka. Molimo 
