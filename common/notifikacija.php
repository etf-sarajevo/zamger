...
function common_notifikacija() {
	...
	if ($tip==3){
		$q243 = myquery("INSERT INTO log (vrijeme, userid, dogadjaj, nivo) VALUES 
							(NOW(), $userid, 'procitana_notifikacija', '2')");
	}

	else if ($tip==4){
			$q248 = myquery("INSERT INTO log (vrijeme, userid, dogadjaj, nivo) VALUES 
							(NOW(), $userid, 'procitana_poruka', '2')");
	}
	
	else if ($tip==1){
	
		$q245 = myquery("select UNIX_TIMESTAMP(vrijeme) from log where userid=$userid AND dogadjaj='procitana_notifikacija' order by vrijeme desc");
...
and UNIX_TIMESTAMP(zk.vrijeme)>$vrijeme order by zk.id desc limit 5");
		...
		$q246 = myquery("select UNIX_TIMESTAMP(vrijeme) from log where userid=$userid AND dogadjaj='procitana_poruka' order by vrijeme desc");
...
// Studij koji student trenutno sluša
	...
	// Poruka je ok
			if (++$br > 5) break; // Nema smisla da gledamo dalje
