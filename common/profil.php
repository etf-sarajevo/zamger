...
function get_user_link($id) {
	...
	if (!$users[$id]) {
		...
		if (mysql_num_rows($q20)>0) {
			$imeprezime = urlencode(mysql_result($q20,0,0)." ".mysql_result($q20,0,1));
...
function get_pk_link($id) {
	...
	if (!$predmeti[$id]) {
		...
		if (mysql_num_rows($q30)>0) {
			$predmeti[$id] = "<a href=\"?sta=nastavnik/predmet&amp;predmet=".mysql_result($q30,0,0)."&ag=".mysql_result($q30,0,2)."\" target=\"_new\">".mysql_result($q30,0,1)."</a>";
		} else return $id;
	}
	return $predmeti[$id];
}
...
function get_predmet_link($id) {
	...
	if (!$predmeti[$id]) {
		...
		if (mysql_num_rows($q40)>0) {
			$predmeti[$id] = "<a href=\"?sta=nastavnik/predmet&amp;predmet=$id&amp;ag=$aktuelna_ag\" target=\"_new\">".mysql_result($q40,0,0)."</a>";
		} else return $id;
	}
	return $predmeti[$id];
}
...
if ($_POST['subakcija'] == "potvrda" && check_csrf_token()) {
	...
	if ($adresa_mjesto != "") {
		...
		if (mysql_num_rows($q3)<1) {
			$q4 = myquery("insert into mjesto set naziv='$adresa_mjesto', opcina=$opcina_rodjenja, drzava=1");
			$q3 = myquery("select id from mjesto where naziv='$adresa_mjesto'");
...
if ($_POST['subakcija']=="postavisliku" && check_csrf_token()) {
	...
	if ($slika && (file_exists($slika))) {
		...
		if (mysql_num_rows($q300)>0) {
			...
			$q320 = myquery("insert into promjena_podataka select 0, $userid, ime, prezime, imeoca, prezimeoca, imemajke, prezimemajke, spol, brindexa, datum_rodjenja, mjesto_rodjenja, nacionalnost, drzavljanstvo, boracke_kategorije, jmbg, adresa, adresa_mjesto, telefon, kanton, '$slikabaza', NOW() from osoba where id=$userid");
		}
