...
let createEventRequest = function(url, method, edit = false){
    ...
    if(students === 2){ // We need to perform extra checks for date and time format
        ...
        if (maxStudents == 0) {
            $.notify("Maksimalan broj studenata mora biti veći od nula", 'warn');
            return;
        }
...
$("body").on('click', '.students-list', function () { // Hide pop-up for event
    window.open("?sta=izvjestaj/termini_ispita&termin=" + edit_event);
});