...
$result = db_query('SELECT DISTINCT o.id, o.ime, o.prezime, o.email, o.spol FROM osoba AS o, angazman AS a, akademska_godina AS ag WHERE a.osoba = o.id AND a.angazman_status=1 AND a.akademska_godina=ag.id AND ag.aktuelna=1');

if (!$result) {
	die('Invalid query: ' . db_error());
}

while ($row = db_fetch_assoc($result)) { //otvori while($row)
	...
	$q100 = db_query("SELECT p.id, p.naziv, ag.id
FROM predmet AS p, angazman AS a, akademska_godina AS ag
WHERE a.osoba = {$row['id']} AND a.predmet=p.id AND a.akademska_godina=ag.id AND ag.aktuelna=1 AND a.angazman_status=1 ORDER BY p.naziv") or die ("SQL greska: ".db_error());
	while ($r100 = db_fetch_row($q100)) {
		...
		$q110 = db_query("SELECT o.id AS student_id, o.ime AS ime, o.prezime AS prezime, o.brindexa AS brindexa, o.email AS email, ko.ocjena AS ocjena, UNIX_TIMESTAMP(ko.datum) AS datum
		...
		ORDER BY prezime, ime") or die ("SQL greska: ".db_error());
		...
		while ($row2 = db_fetch_assoc($q110)) { //otvori while($row2)
			...
			$q = db_query("SELECT l.userid, o.ime, o.prezime FROM log AS l, osoba AS o WHERE l.userid=o.id AND l.dogadjaj='{$logzapis}' AND DATE_SUB(NOW(),INTERVAL $intervalDana DAY) <= l.vrijeme") or die ("SQL greska 1: ".db_error());
			if (db_num_rows($q)>0) {
				$unosilac = db_result($q,0,1)." ".db_result($q,0,2);
			}
			if ($unosilac=="") {
				$logzapis2 = " u ".$row2["ocjena"]." (predmet pp".$r100[0].", student u".$row2["student_id"].")";
				$q2 = db_query("SELECT l.userid, o.ime, o.prezime FROM log AS l, osoba AS o WHERE l.userid=o.id AND l.dogadjaj LIKE '%{$logzapis2}%' AND DATE_SUB(NOW(),INTERVAL $intervalDana DAY) <= l.vrijeme") or die ("SQL greska 2: ".db_error());
				if (db_num_rows($q2)>0) {
					$unosilac = db_result($q2,0,1)." ".db_result($q2,0,2);
				}
			}
			if ($unosilac=="") {
				$logzapis2 = "masovno upisane ocjene na predmet pp".$r100[0];
				$q2 = db_query("SELECT l.userid, o.ime, o.prezime FROM log AS l, osoba AS o WHERE l.userid=o.id AND l.dogadjaj='$logzapis2' AND UNIX_TIMESTAMP(l.vrijeme)={$row2['datum']}") or die ("SQL greska 3: ".db_error());
				if (db_num_rows($q2)>0) {
					$unosilac = db_result($q2,0,1)." ".db_result($q2,0,2);
				} 
				else {
