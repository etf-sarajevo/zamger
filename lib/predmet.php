...
function kreiraj_ponudu_kursa($predmet, $studij, $semestar, $ag, $obavezan, $ispis, $pasos_predmeta) {
	...
	if (db_num_rows($q80)==0) {
		...
		$stara_ag = db_result($q80,0,0);
		if ($ispis) print "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;-- Dodajem slog u aktivnost_agp<br>\n";
		else $q90 = db_query("insert into aktivnost_agp SELECT aktivnost_predmet, $ag, $predmet FROM aktivnost_agp WHERE akademska_godina=$stara_ag AND predmet=$predmet");
	}
