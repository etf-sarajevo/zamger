...
function predmeti_na_planu($plan_studija) 
{
	$rezultat = array();
	$q10 = db_query("select semestar, obavezan, pasos_predmeta, plan_izborni_slot from plan_studija_predmet where plan_studija=$plan_studija and semestar=$sem");
	while (db_fetch4($q10, $semestar, $obavezan, $pasos_predmeta, $plan_izborni_slot)) {
		if ($obavezan == 1) { // obavezan
			$rezultat[] = db_query_assoc("SELECT predmet, naziv, ects FROM pasos_predmeta WHERE id=$pasos_predmeta");
