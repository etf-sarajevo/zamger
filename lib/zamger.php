...
function zamgerlog2($tekst, $objekat1 = 0, $objekat2 = 0, $objekat3 = 0, $blob = "") {
	...
	if ($objekat1 !== intval($objekat1) || $objekat2 !== intval($objekat2) || $objekat3 !== intval($objekat3)) {
		$q5 = db_query("INSERT INTO log2 SELECT 0,NOW(), ".intval($userid).", m.id, d.id, 0, 0, 0, 0, '$ip_adresa' FROM log2_modul AS m, log2_dogadjaj AS d WHERE m.naziv='$sta' AND d.opis='poziv zamgerlog2 funkcije nije ispravan'", true);
		// Dodajemo blob
...
// Briše cachiranu verziju izvještaja izvjestaj/predmet
// Izvještaj će biti regenerisan prilikom sljedećeg otvaranja

function generisi_izvjestaj_predmet($predmet, $ag) {
