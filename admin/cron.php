...
function admin_cron() {
	...
	if ($_REQUEST['akcija'] == "log") {
		$cron = intval($_REQUEST['cron']);
		$id = intval($_REQUEST['id']);
		if ($id>0) $sqladd = "AND id=$id";
		$q20 = myquery("SELECT id, izlaz, return_value, UNIX_TIMESTAMP(vrijeme) FROM cron_rezultat WHERE cron=$cron $sqladd ORDER BY vrijeme DESC");
		
		?>
		<p>Vrijeme izvršenja: <b><?=date("d.m.Y. H:i:s", mysql_result($q20,0,3)) ?></b></p>
		<p>Povratna vrijednost: <b><?=mysql_result($q20,0,2)?></b></p>
		<p>Izlaz:</p>
		<pre><?=mysql_result($q20,0,1)?></pre>
		<hr>
	...
	$q10 = myquery("select id, path, aktivan, godina, mjesec, dan, sat, minuta, sekunda, UNIX_TIMESTAMP(zadnje_izvrsenje), UNIX_TIMESTAMP(sljedece_izvrsenje) FROM cron");
